...
void PredictionUpdate(const Matrix<2> &u, Matrix<3> &xhat, Matrix<3,3> &Phat) {
   ...
   float arrayf[3][1]{{ TSAMPLE*cos(xhat(2))*u(0)+xhat(0) }, //Provide the nonlinear dynamics equation for each state
                      { TSAMPLE*sin(xhat(2))*u(0)+xhat(1) },
                      { TSAMPLE*u(1)+xhat(2) }};
...
void CorrectionUpdate(const Matrix<2> &y, Matrix<3> &xhat, Matrix<3,3> &Phat, Matrix<2> &nu, Matrix<2,2> &S) {
   ...
   float arrayJh[2][3]{{-1/cos(xhat(2)), 0, -sin(xhat(2))*xhat(0)/(cos(xhat(2))*cos(xhat(2)))}, //Provide here the element values of state-space matrix C
                       {0, -1/cos(xhat(2)), ((beta/(cos(xhat(2))*cos(xhat(2))))-(xhat(1)*sin(xhat(2))/(cos(xhat(2))*cos(xhat(2)))))}};
   Matrix<2,3> C = arrayJh;
  
   // Evaluate measurement equation
   float arrayh[2][1]{{ -xhat(0)/cos(xhat(2)) - (alpha - front_offset) },
                      { (beta*sin(xhat(2))-xhat(1))/cos(xhat(2)) - (gamma - side_offset) }};
   Matrix<2> h = arrayh;
