...
void Robot::control() {
  ...
  if(KalmanFilterEnabled()) {   // only do this if Kalman filter is enabled (triggered by pushing 'Button 1' in QRoboticsCenter)
    ...
    if(trajectory.hasMeasurements()){
      ...
      writeValue(0, measurements(0));
      writeValue(1, measurements(1));
    }
    ...
    // // Useful outputs to QRC for assignment questions
     ...
     writeValue(5, _Phat(0,0));
     writeValue(6, _Phat(1,0));
     writeValue(7, _Phat(1,1));
     writeValue(8, _Phat(2,0));
     writeValue(9, _Phat(2,1));
     writeValue(10, _Phat(2,2));
  }
...
void Robot::resetController(){
  ...
  errorA = 0.0;       // stores e[k-1] of motor A during iteration
  errorB = 0.0;       // stores e[k-1] of motor B during iteration
  
  controlA = 0.0;     // stores u[k-1] of motor A during iteration
  controlB = 0.0;     // stores u[k-1] of motor B during iteration
